...
public class CreateEnergyFlowNodes extends CustomJavaAction<java.lang.Void>
{
	...
	public java.lang.Void executeAction() throws Exception
	{
		...
		Core.getLogger("CreateEnergyFlowNodes").info(
			"🚀 Starting CreateEnergyFlowNodes - EnergyType: " + EnergyType + ", MetricType: " + MetricType);
		...
		Core.getLogger("CreateEnergyFlowNodes").info("📊 Found " + allLevels.size() + " levels");
		
		// 3. Récupérer TOUTES les variables (pour validation si nécessaire)
		List<Variable> allVariables = Variable.load(context, "");
		Core.getLogger("CreateEnergyFlowNodes").info("🔧 Found " + allVariables.size() + " variables");
		...
		if (!energyFlowNodes.isEmpty()) {
			...
			Core.getLogger("CreateEnergyFlowNodes").info(
				"✅ Successfully created " + energyFlowNodes.size() + " EnergyFlowNodes");
		}
	...
	private Map<Long, List<Asset>> organizeAssetsByLevel(List<Asset> allAssets, Map<Long, Level> levelMap) throws Exception {
		...
		for (Map.Entry<Long, List<Asset>> entry : assetsByLevel.entrySet()) {
			...
			Core.getLogger("CreateEnergyFlowNodes").info(
				"📋 Level '" + levelName + "': " + entry.getValue().size() + " assets");
		}
	...
	private List<IMendixObject> createAllFlows(IContext context, Map<Long, List<Asset>> assetsByLevel, Map<Long, Level> levelMap) throws Exception {
		...
		for (int i = 0; i < sortedLevels.size() - 1; i++) {
			...
			if (currentAssets != null && nextAssets != null && !currentAssets.isEmpty() && !nextAssets.isEmpty()) {
				Core.getLogger("CreateEnergyFlowNodes").info(
					"🔗 Creating flows from " + currentLevel.getName() + " to " + nextLevel.getName());
	...
	/**
	 * Créer des flux entre deux niveaux
	 */
	private List<IMendixObject> createFlowsBetweenLevels(IContext context, List<Asset> sourceAssets, List<Asset> targetAssets) throws Exception {
		...
		Core.getLogger("CreateEnergyFlowNodes").info("🔍 DEBUG createFlowsBetweenLevels - Source assets: " + sourceAssets.size() + ", Target assets: " + targetAssets.size());
		
		for (Asset sourceAsset : sourceAssets) {
			Core.getLogger("CreateEnergyFlowNodes").info("🔍 DEBUG createFlowsBetweenLevels - Processing source asset: " + sourceAsset.getNom());
			...
			Core.getLogger("CreateEnergyFlowNodes").info("🔍 DEBUG createFlowsBetweenLevels - Found " + children.size() + " children for " + sourceAsset.getNom());
			
			if (children.isEmpty()) {
				Core.getLogger("CreateEnergyFlowNodes").info("🔍 DEBUG createFlowsBetweenLevels - No children found for " + sourceAsset.getNom() + ", skipping");
			...
			for (Asset child : children) {
				...
				Core.getLogger("CreateEnergyFlowNodes").info("🔍 DEBUG createFlowsBetweenLevels - Creating flow: " + sourceAsset.getNom() + " → " + child.getNom() + " (Child value: " + childValue + ", %: " + percentage + ")");
				...
				Core.getLogger("CreateEnergyFlowNodes").info(
					"✅ Flow: " + sourceAsset.getNom() + " → " + child.getNom() + 
					" (Value: " + childValue + ", %: " + percentage + ")");
			}
		}
		
		Core.getLogger("CreateEnergyFlowNodes").info("🔍 DEBUG createFlowsBetweenLevels - Total flows created: " + flowNodes.size());
		return flowNodes;
	}
	
	/**
	 * Vérifier si target est un enfant direct de source
	 */
	private boolean isDirectChild(IContext context, Asset target, Asset source) {
		try {
			...
			if (isChild) {
				Core.getLogger("CreateEnergyFlowNodes").info("🔍 DEBUG isDirectChild - ✅ " + target.getNom() + " is child of " + source.getNom());
			} else {
				Core.getLogger("CreateEnergyFlowNodes").info("🔍 DEBUG isDirectChild - ❌ " + target.getNom() + " is NOT child of " + source.getNom() + " (parent: " + (parent != null ? parent.getNom() : "null") + ")");
			}
			
			return isChild;
		} catch (Exception e) {
			Core.getLogger("CreateEnergyFlowNodes").error("❌ Error in isDirectChild for " + target.getNom() + " → " + source.getNom() + ": " + e.getMessage());
			return false;
		}
	}
	
	/**
	 * Récupérer la valeur de l'asset selon EnergyType et MetricType
	 */
	private java.math.BigDecimal getAssetValue(Asset asset) {
		try {
			if (EnergyType == null || MetricType == null) {
				Core.getLogger("CreateEnergyFlowNodes").warn("🔍 DEBUG getAssetValue - EnergyType or MetricType is null: EnergyType=" + EnergyType + ", MetricType=" + MetricType);
				return java.math.BigDecimal.ZERO;
			}
			
			Core.getLogger("CreateEnergyFlowNodes").info("🔍 DEBUG getAssetValue - Asset: " + asset.getNom() + ", EnergyType: " + EnergyType + ", MetricType: " + MetricType);
			
			java.math.BigDecimal value = java.math.BigDecimal.ZERO;
			
			// Utiliser directement les enums pour un mapping plus sûr
			if (MetricType == smart.proxies.MetricType.Conso) {
				switch (EnergyType) {
					case Elec:
						value = asset.getConsoTotalElec();
						Core.getLogger("CreateEnergyFlowNodes").info("🔍 DEBUG getAssetValue - Conso Elec case, value: " + value);
						break;
					case Gaz:
						value = asset.getConsoTotalGaz();
						Core.getLogger("CreateEnergyFlowNodes").info("🔍 DEBUG getAssetValue - Conso Gaz case, value: " + value);
						break;
					case Eau:
						value = asset.getConsoTotalEau();
						Core.getLogger("CreateEnergyFlowNodes").info("🔍 DEBUG getAssetValue - Conso Eau case, value: " + value);
						break;
					case Air:
						value = asset.getConsoTotalAir();
						Core.getLogger("CreateEnergyFlowNodes").info("🔍 DEBUG getAssetValue - Conso Air case, value: " + value);
						break;
					default:
						Core.getLogger("CreateEnergyFlowNodes").warn("⚠️ No mapping for Conso + " + EnergyType + " (Asset: " + asset.getNom() + ")");
						break;
				}
				...
				switch (EnergyType) {
					case Elec:
						...
						Core.getLogger("CreateEnergyFlowNodes").info("🔍 DEBUG getAssetValue - IPE Elec case, value: " + value);
						break;
					case Gaz:
						value = asset.getIPEGaz();
						Core.getLogger("CreateEnergyFlowNodes").info("🔍 DEBUG getAssetValue - IPE Gaz case, value: " + value);
						break;
					case Eau:
						value = asset.getIPEEau();
						Core.getLogger("CreateEnergyFlowNodes").info("🔍 DEBUG getAssetValue - IPE Eau case, value: " + value);
						break;
					case Air:
						value = asset.getIPEAir();
						Core.getLogger("CreateEnergyFlowNodes").info("🔍 DEBUG getAssetValue - IPE Air case, value: " + value);
						break;
					default:
						Core.getLogger("CreateEnergyFlowNodes").warn("⚠️ No mapping for IPE + " + EnergyType + " (Asset: " + asset.getNom() + ")");
						break;
				}
				...
				switch (EnergyType) {
					case Elec:
						...
						Core.getLogger("CreateEnergyFlowNodes").info("🔍 DEBUG getAssetValue - IPE_kg Elec case, value: " + value);
						break;
					case Gaz:
						value = asset.getIPEGazKg();
						Core.getLogger("CreateEnergyFlowNodes").info("🔍 DEBUG getAssetValue - IPE_kg Gaz case, value: " + value);
						break;
					case Eau:
						value = asset.getIPEEauKg();
						Core.getLogger("CreateEnergyFlowNodes").info("🔍 DEBUG getAssetValue - IPE_kg Eau case, value: " + value);
						break;
					case Air:
						value = asset.getIPEAirKg();
						Core.getLogger("CreateEnergyFlowNodes").info("🔍 DEBUG getAssetValue - IPE_kg Air case, value: " + value);
						break;
					default:
						Core.getLogger("CreateEnergyFlowNodes").warn("⚠️ No mapping for IPE_kg + " + EnergyType + " (Asset: " + asset.getNom() + ")");
						break;
				}
			} else if (MetricType == smart.proxies.MetricType.Prod) {
				// Production n'a pas de type d'énergie spécifique (EnergyType.None)
				value = asset.getProdTotal();
				Core.getLogger("CreateEnergyFlowNodes").info("🔍 DEBUG getAssetValue - Prod case, value: " + value);
			} else if (MetricType == smart.proxies.MetricType.Prod_kg) {
				// Production kg n'a pas de type d'énergie spécifique (EnergyType.None)
				value = asset.getProdTotalKg();
				Core.getLogger("CreateEnergyFlowNodes").info("🔍 DEBUG getAssetValue - Prod_kg case, value: " + value);
			} else {
				Core.getLogger("CreateEnergyFlowNodes").warn("⚠️ No mapping for MetricType: " + MetricType + " + EnergyType: " + EnergyType + " (Asset: " + asset.getNom() + ")");
			}
			
			java.math.BigDecimal finalValue = value != null ? value : java.math.BigDecimal.ZERO;
			Core.getLogger("CreateEnergyFlowNodes").info("🔍 DEBUG getAssetValue - Final value for " + asset.getNom() + ": " + finalValue);
			return finalValue;
			
		} catch (Exception e) {
			Core.getLogger("CreateEnergyFlowNodes").error("❌ Error getting value for " + asset.getNom() + ": " + e.getMessage());
			return java.math.BigDecimal.ZERO;
		}
	}
	...
	private java.math.BigDecimal calculatePercentage(java.math.BigDecimal value, java.math.BigDecimal total) {
		if (total.compareTo(java.math.BigDecimal.ZERO) == 0) {
			...
			// Cela est plus logique que d'attribuer 100% à chaque enfant
			Core.getLogger("CreateEnergyFlowNodes").info("🔍 DEBUG calculatePercentage - Total is 0, using 0% for logical consistency");
			return java.math.BigDecimal.ZERO;
		}
