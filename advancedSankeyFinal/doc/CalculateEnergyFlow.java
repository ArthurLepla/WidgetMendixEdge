...
public class CalculateEnergyFlow extends CustomJavaAction<java.lang.Void>
{
	...
	public java.lang.Void executeAction() throws Exception
	{
		...
		Core.getLogger("CalculateEnergyFlow").info(
			"🚀 Starting CalculateEnergyFlow - EnergyType: " + EnergyType + 
			", MetricType: " + MetricType);
		
		// Récupérer tous les EnergyFlowNodes existants
		List<EnergyFlowNode> energyFlowNodes = EnergyFlowNode.load(context, "");
		if (energyFlowNodes.isEmpty()) {
			Core.getLogger("CalculateEnergyFlow").warn("Aucun EnergyFlowNode trouvé dans la base de données");
			return null;
		}
		
		Core.getLogger("CalculateEnergyFlow").info("📊 Found " + energyFlowNodes.size() + " EnergyFlowNodes to update");
		...
		for (EnergyFlowNode flowNode : energyFlowNodes) {
			try {
				// Récupérer les assets source et target
				...
				Core.getLogger("CalculateEnergyFlow").error("❌ Error processing EnergyFlowNode: " + e.getMessage(), e);
			}
		}
		...
		for (Map.Entry<Long, List<EnergyFlowNode>> entry : flowsByParent.entrySet()) {
			...
			if (totalChildren.compareTo(java.math.BigDecimal.ZERO) > 0) {
				...
				// Aucun flux enfant non‑nul: 0% pour tous
				for (EnergyFlowNode childFlow : childrenFlows) {
		...
		Core.getLogger("CalculateEnergyFlow").info("✅ CalculateEnergyFlow completed successfully");
	...
	/**
	 * Calcule la valeur d'un asset via l'API IIH pour le MetricType et EnergyType spécifiés
	 */
		...
		try {
			...
			if (matchingVariable == null) {
				Core.getLogger("CalculateEnergyFlow").warn(
					"Aucune variable trouvée pour Asset: " + asset.getNom() + 
			...
			Core.getLogger("CalculateEnergyFlow").info(
				"📊 Calculated total value for " + asset.getNom() + ": " + totalValue);
	...
	private Variable findMatchingVariable(IContext context, Asset asset) throws Exception {
		// Récupérer toutes les variables de l'asset
	...
	/**
	 * Récupère la valeur actuelle d'un asset selon EnergyType et MetricType
	 */
	private java.math.BigDecimal getAssetValue(Asset asset) {
		try {
			java.math.BigDecimal value = java.math.BigDecimal.ZERO;
			
			// Mapping basé sur les enums (même logique que dans CreateEnergyFlowNodes)
	...
	private java.math.BigDecimal calculatePercentage(java.math.BigDecimal value, java.math.BigDecimal total) {
		if (total.compareTo(java.math.BigDecimal.ZERO) == 0) {
			...
			Core.getLogger("CalculateEnergyFlow").info(
				"🔍 calculatePercentage - Total is 0, using 100% for test environment");
