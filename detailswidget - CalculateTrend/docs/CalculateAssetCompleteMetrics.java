...
public class CalculateAssetCompleteMetrics extends CustomJavaAction<java.util.List<IMendixObject>>
{
	...
	public java.util.List<IMendixObject> executeAction() throws Exception
	{
		...
		for (String assetName : assetNames) {
			...
			if (IncludeProduction != null && IncludeProduction) {
				Core.getLogger("CalculateAssetCompleteMetrics").info(
					"🏭 [Asset: " + assetName + "] Production requested, adding Prod metric");
				metricsToCalculate.add(MetricType.Prod);
				
				// Only add Prod_kg if variables exist for this type
				// Pour Production, chercher EnergyType = None, pas EnergyType demandé
				...
				if (!prodKgVars.isEmpty()) {
					Core.getLogger("CalculateAssetCompleteMetrics").info(
						"🏭 [Asset: " + assetName + "] Found " + prodKgVars.size() + " Prod_kg variables, adding Prod_kg metric");
					metricsToCalculate.add(MetricType.Prod_kg);
				} else {
					Core.getLogger("CalculateAssetCompleteMetrics").warn(
						"⚠️ [Asset: " + assetName + "] No Prod_kg variables found with EnergyType = None");
				}
			} else {
				Core.getLogger("CalculateAssetCompleteMetrics").info(
					"🏭 [Asset: " + assetName + "] Production not requested (IncludeProduction = " + IncludeProduction + ")");
			}
			...
			for (MetricType metricType : metricsToCalculate) {
				...
				// Pour Production, utiliser EnergyType de la variable (None), pas EnergyType demandé
		...
		// NOUVEAU : Mettre à jour les totaux sur les Assets (déjà calculés)
			...
			for (IMendixObject calculationIMendixObject : calculationIMendixObjectList) {
				...
				for (IMendixObject valueIMendixObject : valueIMendixObjectList) {
					...
					if (calcValue.getValue() != null) {
						...
						// ✅ Association Variable réactivée pour permettre l'affichage des métriques
	...
	/**
	 * Vérifie la correspondance EnergyType avec logique flexible pour Production
	 */
	 ...
	 * S'adapte automatiquement à tous les attributs de totaux disponibles
	 */
	private static class AssetTotals {
		...
		void addMetricTotal(smart.proxies.EnergyType energyType, MetricType metricType, double value) {
			// Utiliser des clés plus spécifiques pour distinguer les modes
			String energyTypeStr = (energyType != null) ? energyType.toString() : "None";
			String key = energyTypeStr + "_" + metricType.toString();  // Plus spécifique
			
			// Log de debug pour tracer les valeurs calculées
			Core.getLogger("CalculateAssetCompleteMetrics").info(
				"🔢 [Asset: " + asset.getNom() + "] Adding metric: " + key + " = " + value);
		...
		void applyTotalsToAsset() {
			try {
				Core.getLogger("CalculateAssetCompleteMetrics").info(
					"🔄 [Asset: " + asset.getNom() + "] Applying totals to asset...");
				...
				for (Map.Entry<String, Double> entry : totalsMap.entrySet()) {
					...
					if (parts.length == 2) {
						...
						Core.getLogger("CalculateAssetCompleteMetrics").info(
							"📝 [Asset: " + asset.getNom() + "] Processing: " + key + " = " + value);
				...
				Core.getLogger("CalculateAssetCompleteMetrics").info(
					"✅ [Asset: " + asset.getNom() + "] Totals applied successfully");
			} catch (Exception e) {
				Core.getLogger("CalculateAssetCompleteMetrics").error(
		...
		/**
		 * Met à jour l'attribut Asset correspondant avec conversion BigDecimal
		 */
		private void updateAssetAttribute(String energyType, String metricType, Double value) {
			try {
				...
				// Mapping intelligent basé sur les patterns de nommage
				switch (key) {
					// Consommation par type d'énergie
					case "Elec_Conso":
						asset.setConsoTotalElec(bdValue);
						Core.getLogger("CalculateAssetCompleteMetrics").info(
							"✅ [Asset: " + asset.getNom() + "] Set ConsoTotalElec = " + bdValue);
						break;
					case "Gaz_Conso":
						asset.setConsoTotalGaz(bdValue);
						Core.getLogger("CalculateAssetCompleteMetrics").info(
							"✅ [Asset: " + asset.getNom() + "] Set ConsoTotalGaz = " + bdValue);
						break;
					case "Eau_Conso":
						asset.setConsoTotalEau(bdValue);
						Core.getLogger("CalculateAssetCompleteMetrics").info(
							"✅ [Asset: " + asset.getNom() + "] Set ConsoTotalEau = " + bdValue);
						break;
					case "Air_Conso":
						asset.setConsoTotalAir(bdValue);
						Core.getLogger("CalculateAssetCompleteMetrics").info(
							"✅ [Asset: " + asset.getNom() + "] Set ConsoTotalAir = " + bdValue);
						break;
						
					// IPE par type d'énergie - MODE QUANTITÉ
					case "Elec_IPE":
						asset.setIPEElec(bdValue);  // ✅ Mode Quantité
						Core.getLogger("CalculateAssetCompleteMetrics").info(
							"✅ [Asset: " + asset.getNom() + "] Set IPEElec = " + bdValue);
						break;
					case "Gaz_IPE":
						asset.setIPEGaz(bdValue);  // ✅ Mode Quantité
						Core.getLogger("CalculateAssetCompleteMetrics").info(
							"✅ [Asset: " + asset.getNom() + "] Set IPEGaz = " + bdValue);
						break;
					case "Eau_IPE":
						asset.setIPEEau(bdValue);  // ✅ Mode Quantité
						Core.getLogger("CalculateAssetCompleteMetrics").info(
							"✅ [Asset: " + asset.getNom() + "] Set IPEEau = " + bdValue);
						break;
					case "Air_IPE":
						asset.setIPEAir(bdValue);  // ✅ Mode Quantité
						Core.getLogger("CalculateAssetCompleteMetrics").info(
							"✅ [Asset: " + asset.getNom() + "] Set IPEAir = " + bdValue);
						break;
						
					// IPE par type d'énergie - MODE KG
					case "Elec_IPE_kg":
						asset.setIPEElecKg(bdValue);  // ✅ Mode Kg
						Core.getLogger("CalculateAssetCompleteMetrics").info(
							"✅ [Asset: " + asset.getNom() + "] Set IPEElecKg = " + bdValue);
						break;
					case "Gaz_IPE_kg":
						asset.setIPEGazKg(bdValue);  // ✅ Mode Kg
						Core.getLogger("CalculateAssetCompleteMetrics").info(
							"✅ [Asset: " + asset.getNom() + "] Set IPEGazKg = " + bdValue);
						break;
					case "Eau_IPE_kg":
						asset.setIPEEauKg(bdValue);  // ✅ Mode Kg
						Core.getLogger("CalculateAssetCompleteMetrics").info(
							"✅ [Asset: " + asset.getNom() + "] Set IPEEauKg = " + bdValue);
						break;
					case "Air_IPE_kg":
						asset.setIPEAirKg(bdValue);  // ✅ Mode Kg
						Core.getLogger("CalculateAssetCompleteMetrics").info(
							"✅ [Asset: " + asset.getNom() + "] Set IPEAirKg = " + bdValue);
						break;
						
					// Production - MODE QUANTITÉ
					case "None_Prod":
						asset.setProdTotal(bdValue);  // ✅ Mode Quantité
						Core.getLogger("CalculateAssetCompleteMetrics").info(
							"✅ [Asset: " + asset.getNom() + "] Set ProdTotal = " + bdValue);
						break;
						
					// Production - MODE KG
					case "None_Prod_kg":
						asset.setProdTotalKg(bdValue);  // ✅ Mode Kg
						Core.getLogger("CalculateAssetCompleteMetrics").info(
							"✅ [Asset: " + asset.getNom() + "] Set ProdTotalKg = " + bdValue);
						break;
						
					default:
						Core.getLogger("CalculateAssetCompleteMetrics").warn(
							"❌ [Asset: " + asset.getNom() + "] No asset attribute mapping found for: " + key);
						break;
				}
					
			} catch (Exception e) {
				Core.getLogger("CalculateAssetCompleteMetrics").error(
					"❌ [Asset: " + asset.getNom() + "] Error setting asset attribute " + energyType + "_" + metricType + ": " + e.getMessage());
			}
		}
